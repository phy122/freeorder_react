...
const ProductUpdate = () => {
  return (
    <div className="i-container" layout:fragment="content">
        <h2>상품 수정</h2>
        <form className="form-group" id="pro-update" method="post">
            ...
            <div className="form-group">
                ...
                <div className="plus-box" onclick="selectOptions(this)"><a>+선택</a></div>
                <ul id="optionList"></ul>
                <input type="hidden" name="optionsId" id="optionsId" th:value="${product.optionsId}"/>
            </div>

            <div className="button-group">
                <button type="button" onclick="proDelete()" className="cancel-btn">삭제</button>
                <button type="button" onclick="proUpdate()" className="submit-btn">저장</button>
            </div>
        </form>
    </div>
  )
